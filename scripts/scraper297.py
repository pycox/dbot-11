...
def main(key, com, url):
    ...
    for item in items:
        ...
        location = item.find_element(By.CSS_SELECTOR, "a div.mt-1.text-md span:nth-child(3)").text.strip()
        for str in locations:
            if (str in location):
                data.append(
                    [
                        item.find_element(By.CSS_SELECTOR, "a span.company-link-style").text.strip(),
                        com,
                        location,
                        link,
                    ]
